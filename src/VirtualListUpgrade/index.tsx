...
const VirtualListUpgrade: React.FC<Props> = ({ preHeight = 50, extraRenderCount = 10, components }) => {
  ...
  const getCurrentRenderItems = () => {
    ...
    const [start, end] = getRangeToRender(tops, scrollTop, document.documentElement.offsetHeight - (scrollContainerRef.current?.offsetTop ?? 0), extraRenderCount)
    ...
    const handleScroll = () => {
      flushSync(() => {
        if (scrollContainerRef.current) {
          setScrollTop(window.scrollY - 200 - scrollContainerRef.current.offsetTop)
        }
      })
    }
...
export default VirtualListUpgrade;
